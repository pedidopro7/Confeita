...
export default async function ProducaoPage() {
  ...
  const [ordersResult, productionResult] = await Promise.all([
    supabase.from('orders').select('id,order_number,status,scheduled_at,customer:customers(name),items:order_items(name_snapshot,quantity)').eq('business_id', context.business.id).in('status', ['confirmed', 'production', 'ready']).order('scheduled_at'),
