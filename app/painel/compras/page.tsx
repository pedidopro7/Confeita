...
import { recordPurchaseNormalizedAction } from './actions';
...
export default async function ComprasPage() {
  ...
  return <><PageHeader eyebrow="Abastecimento" title="Compras" description="Registre entradas em qualquer unidade. A Confeita normaliza tudo para o estoque automaticamente." actionHref="/painel/compras/lista" actionLabel="Lista automática" />
    <div className="grid gap-5 xl:grid-cols-[1fr_380px]">
      ...
      <div className="space-y-5"><form action={recordPurchaseNormalizedAction} className="panel p-5"><p className="eyebrow mb-1">Entrada rápida</p><h2 className="mt-0 text-xl font-black text-wine">Registrar compra</h2><div className="space-y-3"><select name="supplier_id" className="input"><option value="">Fornecedor (opcional)</option>{suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select><select required name="inventory_item_id" className="input"><option value="">Item comprado</option>{items.map(i => <option key={i.id} value={i.id}>{i.name} · estoque em {i.base_unit}</option>)}</select><div className="grid grid-cols-2 gap-3"><input required name="quantity" min="0.0001" step="0.0001" type="number" placeholder="Quantidade" className="input" /><select name="unit" required className="input"><option value="g">g</option><option value="kg">kg</option><option value="ml">ml</option><option value="L">L</option><option value="un">un</option></select></div><input required name="total_cost" min="0" step="0.01" type="number" placeholder="Valor total pago" className="input" /><div className="grid grid-cols-2 gap-3"><input name="lot_code" placeholder="Lote" className="input" /><input name="expires_at" type="date" className="input" /></div><select name="payment_method" className="input"><option value="pix">Pix</option><option value="card">Cartão</option><option value="cash">Dinheiro</option><option value="boleto">Boleto</option></select><textarea name="notes" placeholder="Observação da compra (opcional)" className="input min-h-20 py-3" /></div><p className="mb-0 mt-3 text-[10px] leading-4 text-graphite/40">Ex.: compre 2 kg mesmo que o estoque esteja em gramas. O sistema converte para 2.000 g e recalcula o custo médio por grama.</p><button className="mt-4 h-11 w-full rounded-2xl bg-wine text-sm font-bold text-cream">Dar entrada no estoque</button></form>
