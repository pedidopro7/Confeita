...
export default async function ProdutoDetalhePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: product }, { data: variantRows }, { data: groupRows }, { data: recipeRows }] = await Promise.all([
    ...
    supabase.from('product_option_groups').select('id,name,selection_type,min_select,max_select,sort_order,active,options:product_options(id,name,price_delta,recipe_id,active,sort_order)').eq('business_id', context.business.id).eq('product_id', id).order('sort_order'),
    supabase.from('recipes').select('id,name,product_id,product_variant_id,active_version_id,is_complete').eq('business_id', context.business.id).eq('product_id', id)
  ]);
  ...
  const baseRecipe = recipes.find((r) => !r.product_variant_id);
  const recipeByVariant = new Map(recipes.filter((r) => r.product_variant_id).map((r) => [r.product_variant_id as string, r]));
  const recipeById = new Map(recipes.map((r) => [r.id, r]));

  return <>
    <PageHeader
      eyebrow="Catálogo configurável"
      title={product.name}
      description="Tamanhos, escolhas e fórmulas podem mudar o preço e o consumo do estoque sem complicar o pedido."
    />
    ...
    <div className="grid gap-5 xl:grid-cols-[1fr_370px]">
      <div className="space-y-5">
        <section className="panel p-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="eyebrow mb-1">Fórmula principal</p>
              <h2 className="m-0 text-xl font-black text-wine">Receita secreta do produto</h2>
            </div>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            <Info label="Preço base" value={money(product.base_price)} />
            <Info label="Venda" value={product.sale_unit || 'un'} />
            <Info label="Rendimento base" value={`${numberPt(product.recipe_output_qty)} ${product.recipe_output_unit}`} />
          </div>
        </section>

        <section className="panel overflow-hidden">
          <div className="flex items-center justify-between border-b border-wine/10 px-5 py-4">
            <div><p className="eyebrow mb-1">Tamanhos e formatos</p><h2 className="m-0 text-lg font-black text-wine">Variantes</h2></div>
            <span className="rounded-full bg-cream px-3 py-1 text-[10px] font-bold text-wine">{variants.length}</span>
          </div>
          {variants.length === 0 ? <div className="p-7 text-center text-xs text-graphite/45">Use variantes para 1kg, 2kg, 3kg, caixas de 6/12 unidades ou outros formatos.</div> : <div className="divide-y divide-wine/5">{variants.map((variant) => {
            ...
            return <div key={variant.id} className="grid gap-3 px-5 py-4 md:grid-cols-[1fr_130px_180px] md:items-center">
              <div><p className="m-0 text-sm font-black text-graphite">{variant.name}</p><p className="m-0 mt-1 text-xs text-graphite/40">{variant.sku ? `SKU ${variant.sku} · ` : ''}{numberPt(variant.recipe_output_qty)} {variant.recipe_output_unit}</p></div>
              <strong className="text-sm text-wine">{money(variant.price)}</strong>
              {recipe?.is_complete ? <span className="inline-flex items-center gap-2 text-xs font-bold text-success"><LockKeyhole size={13}/> Receita protegida</span> : <Link href={`/painel/cofre/nova?produto=${product.id}&variante=${variant.id}`} className="inline-flex items-center gap-2 text-xs font-bold text-terracotta"><Plus size={13}/> Vincular fórmula</Link>}
            </div>;
          })}</div>}
        </section>
        ...
        <section className="space-y-3">
          ...
          {groups.length === 0 ? <div className="panel p-8 text-center text-xs text-graphite/45">Ex.: Massa, Recheio, Cobertura, Decoração e Extras.</div> : groups.map((group) => {
            const options = (group.options ?? []).filter((o) => o.active).sort((a, b) => a.sort_order - b.sort_order);
            return <article key={group.id} className="panel overflow-hidden">
              <div className="flex flex-col gap-2 border-b border-wine/10 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div><p className="m-0 text-sm font-black text-wine">{group.name}</p><p className="m-0 mt-1 text-[10px] uppercase tracking-wider text-graphite/35">{group.selection_type === 'multiple' ? 'Múltiplas escolhas' : 'Escolha única'} · mínimo {group.min_select}{group.max_select ? ` · máximo ${group.max_select}` : ''}</p></div>
                <span className="text-xs font-bold text-graphite/40">{options.length} opções</span>
              </div>
              <div className="divide-y divide-wine/5">{options.length === 0 ? <div className="px-5 py-5 text-xs text-graphite/40">Nenhuma opção cadastrada.</div> : options.map((option) => {
                ...
                return <div key={option.id} className="flex flex-wrap items-center gap-3 px-5 py-3 text-xs">
                  <span className="min-w-0 flex-1 font-bold text-graphite">{option.name}</span>
                  <span className="font-bold text-terracotta">{Number(option.price_delta) === 0 ? 'sem acréscimo' : `${Number(option.price_delta) > 0 ? '+' : ''}${money(option.price_delta)}`}</span>
                  <span className={`inline-flex items-center gap-1 font-bold ${linkedRecipe ? 'text-wine' : 'text-graphite/30'}`}><LockKeyhole size={11}/>{linkedRecipe ? 'Receita vinculada' : 'Sem receita extra'}</span>
                </div>;
              })}</div>
            </article>;
          })}
      ...
      <aside className="space-y-5">
        <form action={updateProductCatalogAction} className="panel p-5">
          ...
          <p className="eyebrow mb-1">Dados do produto</p><h2 className="mt-0 text-lg font-black text-wine">Configuração geral</h2>
          <div className="space-y-3">
            <input required name="name" defaultValue={product.name} className="input" />
            <textarea name="description" defaultValue={product.description ?? ''} className="input min-h-20 py-3" placeholder="Descrição" />
            <select name="product_type" defaultValue={product.product_type} className="input"><option value="simple">Simples</option><option value="sized">Por tamanho</option><option value="configurable">Configurável</option><option value="kit">Kit</option></select>
            <div className="grid grid-cols-2 gap-3"><input name="base_price" type="number" min="0" step="0.01" defaultValue={Number(product.base_price)} className="input"/><select name="sale_unit" defaultValue={product.sale_unit} className="input"><option value="un">unidade</option><option value="kg">kg</option><option value="cento">cento</option><option value="caixa">caixa</option><option value="kit">kit</option></select></div>
            <div className="grid grid-cols-2 gap-3"><input name="recipe_output_qty" type="number" min="0.0001" step="0.0001" defaultValue={Number(product.recipe_output_qty)} className="input"/><select name="recipe_output_unit" defaultValue={product.recipe_output_unit} className="input"><option value="un">un</option><option value="g">g</option><option value="kg">kg</option><option value="ml">ml</option><option value="L">L</option></select></div>
          </div>
        ...
        {groups.length > 0 && <form action={createProductOptionAction} className="panel p-5">
          ...
          <div className="space-y-3"><select required name="group_id" className="input"><option value="">Escolha o grupo</option>{groups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select><input required name="name" placeholder="Ex.: Ninho com morango" className="input"/><input name="price_delta" type="number" step="0.01" defaultValue="0" placeholder="Acréscimo" className="input"/><select name="recipe_id" className="input"><option value="">Sem receita extra</option>{recipes.filter((r) => r.is_complete).map((r) => <option key={r.id} value={r.id}>{r.name}</option>)}</select></div>
