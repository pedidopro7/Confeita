...
import {
  BarChart3, Bell, Calculator, CalendarDays, ChevronDown, ClipboardList, Cookie, DollarSign,
  FileText, LockKeyhole, LogOut, Menu, PackageOpen, Plus, Search, Settings, ShoppingBasket, Sparkles,
  UsersRound, X
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-cream md:grid md:grid-cols-[248px_1fr]">
      ...
      <aside className="sticky top-0 hidden h-screen border-r border-wine/10 bg-white/70 p-4 backdrop-blur md:flex md:flex-col">
        ...
        <div className="mb-4 grid grid-cols-[1fr_42px] gap-2">
          <form action="/painel/busca" method="get" className="relative">
            <Search size={14} className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-wine/35"/>
            <input name="q" aria-label="Buscar" placeholder="Buscar..." className="h-10 w-full rounded-xl border border-wine/10 bg-white pl-9 pr-3 text-xs outline-none focus:border-terracotta/40"/>
          </form>
          <Link href="/painel/notificacoes" className="flex h-10 items-center justify-center rounded-xl border border-wine/10 bg-white text-wine" aria-label="Avisos"><Bell size={16}/></Link>
        </div>
      ...
      <div className="min-w-0">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-wine/10 bg-cream/90 px-4 backdrop-blur md:hidden">
          <Link href="/painel" className="flex items-center gap-2"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-wine"><Image src="/confeita-mark.svg" alt="Confeita" width={27} height={27} /></div><span className="text-xl font-black text-wine">confeita<span className="text-terracotta">.</span></span></Link>
          <div className="flex items-center gap-2">
            <Link href="/painel/busca" className="flex h-10 w-10 items-center justify-center rounded-xl border border-wine/10 bg-white text-wine" aria-label="Buscar"><Search size={17}/></Link>
            <Link href="/painel/notificacoes" className="flex h-10 w-10 items-center justify-center rounded-xl border border-wine/10 bg-white text-wine" aria-label="Avisos"><Bell size={17}/></Link>
            <button onClick={() => setMenuOpen(true)} className="flex h-10 w-10 items-center justify-center rounded-xl border border-wine/10 bg-white text-wine" aria-label="Abrir menu"><Menu size={19} /></button>
          </div>
        </header>
