...
const nav = [
  ...
  { href: '/painel/produtos', label: 'Produtos', icon: PackageOpen },
  { href: '/painel/cofre', label: 'Cofre', icon: LockKeyhole, secure: true },
  ...
  return (
    <div className="min-h-screen bg-cream md:grid md:grid-cols-[248px_1fr]">
      <aside className="sticky top-0 hidden h-screen border-r border-wine/10 bg-white/70 p-4 backdrop-blur md:flex md:flex-col">
        ...
        <div className="mt-4 rounded-2xl border border-wine/10 bg-cream/65 p-3">
          <div className="mb-3 flex items-center gap-3">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-rose/45 text-xs font-black text-wine">{userName.slice(0, 1).toUpperCase()}</div>
            <div className="min-w-0 flex-1"><p className="m-0 truncate text-xs font-bold text-graphite">{userName}</p><p className="m-0 truncate text-[10px] text-graphite/45">{businessName}</p></div>
            <ChevronDown size={14} className="text-wine/35" />
          </div>
      ...
      <nav className="fixed bottom-3 left-1/2 z-30 flex w-[calc(100%-24px)] max-w-md -translate-x-1/2 items-center justify-around rounded-3xl border border-wine/10 bg-white/95 px-2 py-2 shadow-soft backdrop-blur md:hidden">
        ...
        <Link href="/painel/pedidos/novo" className="-mt-7 flex h-14 w-14 items-center justify-center rounded-2xl bg-wine text-cream shadow-soft" aria-label="Novo pedido"><Plus /></Link>
